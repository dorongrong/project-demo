...
//    }
    ...
    public ChatRoomDto getOrSaveChatRoom(ChatRoomDto chatRoomDto) {
        ...
        //채팅방 요구자(buyer이 아님..)
        User requestUser = userRepository.findByLoginId(chatRoomDto.getSendUserLoginId()).get();
        ...
        if (seller == requestUser) {
            ...
            if (!chatRoomRepository.existsByBuyerIdAndSellerAndItem(chatRoomDto.getBuyerId(), requestUser, item))
            {
                ...
                //아닐경우 채팅방 건내주자
                //상대방 채팅의 chatRead값을 전부 2로 업데이트 후 채팅방을 건내주자
                ChatRoom chatRoom = chatRoomRepository.findByItemAndSellerAndBuyerId(item, requestUser, chatRoomDto.getBuyerId());
                chatRoom.setOtherReadCount(requestUser.getId());

                return new ChatRoomDto(item.getId(), String.valueOf(requestUser.getLoginId()), chatRoom.getBuyer().getLoginId(),
            ...
            if (!chatRoomRepository.existsByBuyerAndSellerAndItem(requestUser, seller, item)) {
                ...
                return new ChatRoomDto(item.getId(), String.valueOf(requestUser.getLoginId()), chatRoom.getSeller().getLoginId(),
